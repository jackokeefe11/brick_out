          ...
          random_boost_source(-config.max_boost, config.max_boost)
{
    for (int i = 0; i < config.brick_cols; i++) {
        for (int j = 0; j < config.brick_rows; j++) {
            ...
            int x = config.side_margin + i * (config.brick_dims().width +
                    config.brick_spacing.width);

            int y = config.top_margin + j * (config.brick_dims().height +
                    config.brick_spacing.height);

            Block brick = Block::from_top_left({x, y}, config.brick_dims());
...
{
    if (ball.live) {
        Ball holder = ball.next(dt);

        if (holder.hits_bottom(config)) {
            ball = Ball(paddle, config);
            return;
        }

        if (holder.hits_top(config)) ball.reflect_vertical();

        if (holder.hits_side(config)) ball.reflect_horizontal();

        if (holder.hits_block(paddle)) ball.reflect_vertical();

        if (holder.destroy_brick(bricks)) {
            ball.reflect_vertical();
            ball.velocity.width += random_boost_source.next();
            holder.destroy_brick(bricks);
        }
        ball = ball.next(dt);
    }
